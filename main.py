...
def main():
    ...
    if logger:
        ...
        if args.attack_type and args.duration and args.threads:
    ...
    if logger:
        ...
        logger.info(f"Duration: {args.duration}")
        logger.info(f"Threads Used: {args.threads}")
        logger.info(f"Threads Ended: {ended_threads}")
        if args.attack_type == "http":
            logger.info(f"Total HTTP Requests: {request_count}")
        else:
            logger.info(f"Estimated SYN Packets: ~{ended_threads * 1000}")
        logger.info("=========== End of Log ===========")
